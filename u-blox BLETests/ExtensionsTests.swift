...
class ExtensionsTests: XCTestCase {
    ...
    func testByteArray() {
        ...
        XCTAssertEqual(Data(bytes: bytes).byteArray, bytes)
    }

    func testIsSerialPortService() {
        XCTAssertEqual(CBUUID(data: Data(bytes: [0x24, 0x56, 0xe1, 0xb9, 0x26, 0xe2, 0x8f, 0x83, 0xe7, 0x44, 0xf3, 0x4f, 0x01, 0xe9, 0xd7, 0x01])).isSerialPortService, true)
        XCTAssertEqual(CBUUID(data: Data(bytes: [0x24, 0x56])).isSerialPortService, false)
    }
...
class MockCharacteristic: CBCharacteristic {
    ...
    override var uuid: CBUUID {
        return CBUUID(data: Data(bytes: bytes))
    }
